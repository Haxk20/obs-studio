			     ...
			     char **out_token)
{
	...
	if (out_path)
		*out_path = g_strdup_printf(REQUEST_PATH, data->sender_name,
					    request_token_count);
}
			     ...
			     char **out_token)
{
	...
	if (out_path)
		*out_path = g_strdup_printf(SESSION_PATH, data->sender_name,
					    session_token_count);
}
...
static dbus_call_data *subscribe_to_signal(obs_pipewire_data *obs_pw,
					   const char *path,
...
static void dbus_call_data_free(dbus_call_data *call)
{
	if (!call)
		return;
...
static void maybe_queue_buffer(obs_pipewire_data *obs_pw)
{
	if (obs_pw->current_pw_buffer) {
		pw_stream_queue_buffer(obs_pw->stream,
				       obs_pw->current_pw_buffer);
		obs_pw->current_pw_buffer = NULL;
	}
}

static void teardown_pipewire(obs_pipewire_data *obs_pw)
{
	maybe_queue_buffer(obs_pw);
...
static void destroy_session(obs_pipewire_data *obs_pw)
{
	...
	g_clear_pointer(&obs_pw->sender_name, g_free);
...
read_metadata:
	...
	if (obs_pw->cursor.visible && obs_pw->cursor.valid) {
		...
		if (bitmap && bitmap->size.width > 0 &&
		    ...
		    spa_pixel_format_to_obs_pixel_format(bitmap->format,
							 &format)) {
	...
	/*
   * Don't immediately queue the buffer now. The buffer will be queued back
	 * in obs_pipewire_render_video().
   */
}
...
static void play_pipewire_stream(obs_pipewire_data *obs_pw)
{
	...
	obs_pw->core = pw_context_connect_fd(
		obs_pw->context, fcntl(obs_pw->pipewire_fd, F_DUPFD_CLOEXEC, 3),
		NULL, 0);
	if (!obs_pw->core) {
	...
	params[0] = spa_pod_builder_add_object(
		...
		SPA_POD_CHOICE_ENUM_Id(
			4, SPA_VIDEO_FORMAT_RGBA, SPA_VIDEO_FORMAT_RGBx,
	...
	blog(LOG_INFO, "[pipewire] Starting monitor screencast…");
					  ...
					  GVariant *parameters, void *user_data)
{
	...
	blog(LOG_INFO, "[pipewire] Monitor selected, setting up screencast");
...
static void start(obs_pipewire_data *obs_pw)
{
	...
	blog(LOG_INFO, "[pipewire] Asking for monitor…");
...
static gboolean init_obs_pipewire(obs_pipewire_data *obs_pw)
{
	...
	obs_pw->sender_name = g_strdup(
		g_dbus_connection_get_unique_name(obs_pw->connection) + 1);

	/* Replace dots by underscores */
	while ((aux = g_strstr_len(obs_pw->sender_name, -1, ".")) != NULL)
		*aux = '_';
